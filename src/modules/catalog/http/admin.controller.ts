...
/**
 * Единица инвентаря — общая форма для списка, поиска по метке и
 * мутаций (`POST /v1/admin/items` отдаёт созданные единицы этой же
 * формой). Описана по типу домена `InventoryItem`, сверена с ответом:
 * все 13 полей совпали.
 */
...
const ItemsResponse = v.object({
  ...
  // и «где вещь BO-0147».
  items: v.optional(v.array(InventoryItemSchema)),
  item: v.optional(v.nullable(InventoryItemSchema)),
