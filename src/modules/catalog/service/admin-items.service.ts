...
/**
 * QR как SVG-строка рядом с каждой единицей.
 *
 * ⚠️ Рисуем на СЕРВЕРЕ, а не тянем библиотеку в браузер: qrcode весит
 * заметно больше, чем готовая разметка на полсотни вещей, а страница
 * и так уже отдаёт список. Плюс одна реализация на печать и на экран —
 * иначе миниатюра и наклейка однажды разойдутся.
 *
...
export async function getItems(
  ...
  return deps.db.tx(session.tenantId, async (c) => {
    ...
    return {
      items: await withQr(await listItems(c, {
