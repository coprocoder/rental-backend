...
export function createApp({ config, deps }: AppOptions): App {
  ...
  /**
   * Тот же API дополнительно на префиксе `/api`.
   *
   * ⚠️ Нужен ради переключения фронта ОДНОЙ переменной окружения.
   * В браузере 84 вызова написаны как `/api/v1/…` — так их формировал
   * Nuxt, где `server/api/` и есть маршрут. Переписать их значит
   * тронуть 39 файлов ради косметики и потерять возможность откатиться
   * той же переменной: старый бэк отвечает только на `/api/v1/…`.
   *
   * ⚠️ Оба префикса ведут в ОДИН набор обработчиков — это псевдоним,
   * а не вторая копия API. Убрать после завершения переезда, вместе
   * с удалением `server/` из Nuxt.
   */
