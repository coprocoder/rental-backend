...
module.exports = {
  forbidden: [
    ...
    },
    {
      ...
      severity: 'error',
      comment:
        'domain МОДУЛЯ не делает ввод-вывод: ни запросов к БД, ни pg, ни внешних систем. '
        + 'Правила предметной области обязаны проверяться без базы — иначе их '
        + 'не покрывают тестами, а значит не покрывают вовсе.\n'
        + '⚠️ src/domain (унаследованный домен, приехавший из Nuxt) под правило НЕ '
        + 'подпадает: там 10 200 строк, принимающих PoolClient напрямую. Переписывать '
        + 'их ради инверсии зависимостей запрещено планом переезда — это работа, '
        + 'которая ничего не чинит и рискует поведением. Модуль, в который такой код '
        + 'переезжает, обязан разделить правила и SQL; проверяется на src/modules/.',
    ...
    },
    {
      ...
      comment:
        ...
        + '⚠️ Встроенные модули Node разрешены: hash-text считает хеш через node:crypto '
        + 'и в браузер не уезжает — он серверный и в сверку копий с фронтом не входит.',
