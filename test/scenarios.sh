...
# ⚠️ База читается ЛОКАЛЬНЫМ docker exec, а запросы идут на $1.
# Значит скрипт запускается ТАМ ЖЕ, где стоит база: локально против
# localhost:3200, на сервере — против localhost:8080, скопировав файл
# туда. Запуск с машины разработчика против удалённого стенда даёт
# 404 на каждом шаге с идентификатором — варианты и филиалы берутся
# из чужой базы. Проверено: так и вышло при первом прогоне на прод.
...
# ─────────────────────── уборка за собой ───────────────────────
#
# ⚠️ Сценарии создают НАСТОЯЩИЕ заказы в той же базе, с которой снят
# эталон ответов. Один прогон ломал 4 эталона из 38 — и это выглядело
# как расхождение в коде, хотя код был ни при чём. Две проверки мешали
# друг другу, и красное переставало что-либо значить (19.46).
#
# ⚠️ Удаляется ТОЛЬКО созданное ЭТИМ прогоном — по телефону, который
# генерируется случайным на каждый запуск. Удалять «всех Сценариев
# Проверка» нельзя: параллельный прогон на том же стенде потерял бы
# свои данные посреди работы.
#
# ⚠️ Порядок от листьев к корню: каскадного удаления заказа нет
# намеренно — «снести заказ одним DELETE» не должно быть однострочником.
if [ -n "${PHONE:-}" ]; then
  q "
    BEGIN;
    CREATE TEMP TABLE mine ON COMMIT DROP AS
      SELECT o.id, o.customer_id FROM rental_order o
      JOIN customer c ON c.id = o.customer_id
      WHERE c.tenant_id = '$TID' AND c.phone = '$PHONE';
    -- ⚠️ Outbox тоже: создание заказа кладёт туда событие со ссылкой
    -- на подтверждение, и оно попадает в счётчик 'сегодня'. Без этой
    -- строки эталон расходился ровно на единицу в outboxPending —
    -- проверено прогоном.
    DELETE FROM outbox WHERE payload->>'orderId' IN (SELECT id::text FROM mine);
    DELETE FROM order_line  WHERE order_id IN (SELECT id FROM mine);
    DELETE FROM order_token WHERE order_id IN (SELECT id FROM mine);
    DELETE FROM consent     WHERE order_id IN (SELECT id FROM mine);
    DELETE FROM agreement   WHERE order_id IN (SELECT id FROM mine);
    DELETE FROM rental_order WHERE id IN (SELECT id FROM mine);
    DELETE FROM consent  WHERE customer_id IN (SELECT DISTINCT customer_id FROM mine);
    DELETE FROM customer WHERE id IN (SELECT DISTINCT customer_id FROM mine);
    COMMIT;
  " > /dev/null 2>&1 && echo "  ✓ данные прогона убраны" \
    || echo "  ⚠️ уборка не прошла — эталон разойдётся, см. 19.46" >&2
fi
